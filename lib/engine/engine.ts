...
import { synthesizeClaudeResearch } from './claude-research'
  ...
  try {
    ...
    if (wantsResearch) {
      try {
        ...
        if (sources.length > 0) {
          ...
          // Sintesis model di-skip di jalur cepat (hemat 5–15 dtk).
          // Evidence mentah sudah cukup untuk model chat utama.
          meta.claudeResearch = { hops: 0, used: false, fastPath: true }
        }
      } catch {
        /* research opsional */
      }
    ...
    if (wantsThinking) {
      try {
        ...
        const thinkResult = await callGroqWithTools(
          ...
          { maxTokens: 1028, timeoutMs: 30_000 },
        )
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      const modelResult = await callGroqWithTools(
        ...
        chatMessages,
        activeTools,
        { maxTokens: 1028, timeoutMs: 45_000 },
      )
    ...
    if (false && finalContent && needsDeepThink(lastUserText)) {
      try {
        const deep = await think10x(lastUserText, {
          kind: 'thinking',
          priorContext: finalContent.slice(0, 400),
          force: true,
        })
        if (deep.usedDeep && deep.content && deep.content.trim().length > finalContent.trim().length) {
    ...
    if (false && finalContent && finalContent.trim().length < 100) {
      try {
        const rolled = await generateRolling(
          ...
          { maxRounds: 2, maxTokens: 1028, temperature: 0.6 },
        )
    ...
    if (wantsCreative && finalContent) {
      try {
        const creativeResult = await callGroqWithTools(
          ...
          [] as GroqToolDefinition[],
          { maxTokens: 1028, temperature: 0.8, timeoutMs: 30_000 },
        )
        ...
        if (false && finalContent.trim().length < 80) {
          try {
            const more = await generateRolling(
              ...
              { maxRounds: 1, maxTokens: 1028, temperature: 0.7 },
            )
    ...
    if (!finalContent.trim()) {
      try {
        const fb = await callGroqWithTools(
          ...
          [] as GroqToolDefinition[],
          { maxTokens: 1028, temperature: 0.3 },
        )
...
export async function runCreative(
  ...
  try {
    ...
    const result = await callGroqWithTools(
      ...
      [] as GroqToolDefinition[],
      { maxTokens: 1028, temperature: 0.9 },
    )
