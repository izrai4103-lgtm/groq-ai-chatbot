...
export async function getUserTokenStatus(key, isLoggedIn) {
  ...
  if (blobEnabled()) {
    if (userMapCache && now - userMapCacheTs <= USER_CACHE_MS) {
      ...
      memoryUserMap = { ...memoryUserMap, ...map }
      userMapCache = map
      userMapCacheTs = now
    }
  }
  ...
  if (isRecValid(rec, now)) {
    // Window masih sama — pakai used yang ada
    // Jika resetAt beda dari windowEnd (legacy), tetap hormati resetAt record
    return {
      isLoggedIn,
      quota,
      used: rec.used,
  ...
  // Window baru / belum ada record — sisa penuh, countdown ke akhir menit
  return {
    isLoggedIn,
    quota,
    used: 0,
...
export async function deductUserTokens(key, isLoggedIn, tokens) {
  ...
  const run = async () => {
    try {
      ...
      const resetAt = valid ? rec.resetAt : end
      const base = valid ? rec.used : 0
      map[key] = { used: base + Math.round(n), resetAt }
      await writeUserMap(map)
      return {
        isLoggedIn,
        quota: isLoggedIn ? USER_QUOTA_LOGIN : USER_QUOTA_GUEST,
        used: map[key].used,
        remaining: Math.max(0, (isLoggedIn ? USER_QUOTA_LOGIN : USER_QUOTA_GUEST) - map[key].used),
        resetAt,
        serverNow: now,
      }
    } catch (e) {
