...
/** Gabungkan hint dari client (localStorage) agar multi-instance tidak rollback used. */
export async function applyClientTokenHint(key, isLoggedIn, hint) {
  if (!key || !hint || typeof hint !== 'object') return getUserTokenStatus(key, isLoggedIn)
  const used = Number(hint.used)
  const resetAt = Number(hint.resetAt)
  if (!Number.isFinite(used) || used < 0) return getUserTokenStatus(key, isLoggedIn)
  const now = nowMs()
  const end = windowEnd(now)
  const rec = memoryUserMap[key]
  const validLocal = isRecValid(rec, now)
  const hintValid = Number.isFinite(resetAt) && resetAt > now
  // Window sama atau hint valid: ambil used tertinggi
  if (hintValid) {
    const base = validLocal && rec.resetAt === resetAt ? Math.max(rec.used || 0, used) : used
    memoryUserMap[key] = { used: base, resetAt }
    userMapCache = { ...memoryUserMap }
    userMapCacheTs = now
    if (blobEnabled()) {
      try { await writeUserMap(memoryUserMap) } catch (e) { /* ignore */ }
    }
  } else if (validLocal) {
    memoryUserMap[key] = { used: Math.max(rec.used || 0, used), resetAt: rec.resetAt }
  } else {
    memoryUserMap[key] = { used, resetAt: end }
  }
  return getUserTokenStatus(key, isLoggedIn)
}
