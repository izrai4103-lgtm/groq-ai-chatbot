...
export function recordUsage(bucket, kind, tokens) {
  ...
  events = prune(events)
  console.error(`[tu] record ${kind} +${n}, events=${events.length}`)
  scheduleFlush()
}
...
async function readRemote() {
  ...
  try {
    ...
    if (!url) { console.error('[tu] read: no url'); return null }
    const { get } = await import('@vercel/blob')
    const res = await get(url, { access: 'private', timeout: 4000 })
    if (!res || typeof res.text !== 'function') { console.error('[tu] read: res kosong'); return null }
    const txt = await res.text()
    if (!txt) { console.error('[tu] read: text kosong'); return null }
    const parsed = JSON.parse(txt)
    if (parsed && Array.isArray(parsed.events)) { console.error(`[tu] read OK: ${parsed.events.length} events`); return parsed }
    console.error('[tu] read: format salah')
  } catch (e) { console.error('[tu] read gagal:', e instanceof Error ? e.message : String(e)) }
  return null
}
...
async function writeRemote(data) {
  ...
  try {
    ...
    blobUrl = res.url
    console.error('[tu] write OK')
  } catch (e) { console.error('[tu] write gagal:', e instanceof Error ? e.message : String(e)) }
}
...
export async function getTokenUsage() {
  ...
  console.error(`[tu] getTokenUsage: used=${used} remaining=${remaining} events=${mergedEvents.length}`)
  return {
