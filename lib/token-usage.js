...
export function recordUsage(bucket, kind, tokens) {
  ...
  events = prune(events)
  console.error(`[token-usage] recordUsage ${kind} +${n} (total ${events.reduce((s,e)=>s+e.tokens,0)})`)
  scheduleFlush()
}
...
async function readRemote() {
  ...
  try {
    ...
    console.error('[token-usage] readRemote: format salah', txt.slice(0, 100))
  } catch (e) { console.error('[token-usage] readRemote gagal:', e instanceof Error ? e.message : String(e)) }
  return null
}
...
async function writeRemote(data) {
  ...
  try {
    ...
    blobUrl = res.url
    console.error(`[token-usage] writeRemote OK (${data.events.length} events)`)
  } catch (e) { console.error('[token-usage] writeRemote gagal:', e instanceof Error ? e.message : String(e)) }
}
