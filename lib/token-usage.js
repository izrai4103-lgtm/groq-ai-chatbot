...
// Direset otomatis tengah malam (UTC).
...
export async function getUserTokenStatus(key, isLoggedIn) {
  ...
  const now = nowMs()
  const rec = map[key]
  if (rec && typeof rec.used === 'number' && rec.resetAt && rec.resetAt > now) {
    return {
      isLoggedIn,
      quota,
      used: rec.used,
...
export async function deductUserTokens(key, isLoggedIn, tokens) {
  ...
  const run = async () => {
    try {
      ...
      const now = nowMs()
      const rec = map[key]
      const valid = rec && typeof rec.used === 'number' && rec.resetAt && rec.resetAt > now
      const resetAt = valid ? rec.resetAt : endOfDayUtc(now)
