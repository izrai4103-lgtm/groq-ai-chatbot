...
function sanitizeInput(text) {
  ...
  // Strip kontrol karakter + zero-width (anti-obfuscation), batasi panjang
  let clean = text
...
async function fetchModel(messages) {
  const { getDefaultKey } = await import('./provider-keys.js')
  const entry = getDefaultKey('chat')

  if (!entry) {
    throw new SandboxError('AI_MODEL_UNAVAILABLE', 'Kunci API model tidak tersedia')
  }
  const apiKey = entry.key
  const GROQ_URL = entry.url
...
export async function executeInSandbox({ messages, ip }) {
  ...
  try {
    ...
    // 3b. Rate limit dulu (hemat jailbreak ML saat flood)
    ...
    // 4. Jailbreak scan (sebelum pesan sampai ke model AI)
