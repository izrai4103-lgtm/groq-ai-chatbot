...
const analyzeWebsiteTool = {
  ...
  function: {
    ...
    description:
      'Buka URL di browser server: ambil judul, deskripsi, heading, teks, screenshot, status HTTP, waktu load, mobile-friendly, error console. Pakai saat user minta portofolio/laporan dari URL. Rangkum fitur website dengan katamu sendiri lalu panggil generate_portfolio_pdf. Jika ok:false, beri tahu user.',
    parameters: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'URL website' },
      },
      required: ['url'],
    },
  },
...
const generatePortfolioPdfTool = {
  ...
  function: {
    ...
    description:
      'Buat PDF portofolio dari data yang terkumpul. Panggil setelah data lengkap; jangan mengarang.',
    parameters: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Nama lengkap' },
        title: { type: 'string', description: 'Jabatan' },
        tagline: { type: 'string' },
        email: { type: 'string' },
        phone: { type: 'string' },
        ...
        projects: {
          ...
          items: {
            ...
            properties: {
              ...
              link: { type: 'string' },
              screenshot: { type: 'string', description: 'base64 screenshot (opsional)' },
            },
            required: ['name'],
          },
        },
        education: {
          ...
          items: {
            type: 'object',
            properties: { degree: { type: 'string' }, school: { type: 'string' }, period: { type: 'string' } },
        ...
        accentColor: { type: 'string', description: 'Hex warna aksen (opsional)' },
      },
      required: ['name', 'title'],
    },
  },
