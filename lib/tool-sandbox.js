...
const { getGroqKeys } = require('./groq-keys.js')
...
PENTING:
- Begitu user minta portofolio/PDF, LANGSUNG panggil tool terkait — jangan menolak, jangan menjelaskan cara kerja, jangan bilang tidak bisa.
- Argumen tool harus JSON yang lengkap dan valid, tanpa teks di luar JSON.
- Kalau pemanggilan tool gagal, coba sekali lagi dengan data yang lebih ringkas.
- Link PDF dari tool sudah lengkap (absolute URL). Sampaikan persis apa adanya — jangan menambah/mengganti domain, jangan potong, jangan ubah formatnya.
`
...
async function fetchGroqOnce({ body, timeoutMs, signal, maxAttempts = 8 }) {
...
async function callGroqOnce({ model, systemPrompt, messages, tools, temperature, maxTokens, timeoutMs }) {
  ...
  try {
    ...
    let res = await fetchGroqOnce({ body: makeBody(temperature), timeoutMs, signal: controller.signal })
    ...
    if (!res.ok) {
      ...
      if (isToolFail) {
        res = await fetchGroqOnce({ body: makeBody(0.2, Math.max(maxTokens, 800)), timeoutMs, signal: controller.signal })
        if (res.ok) {
    ...
    if (truncated && maxTokens < 800) {
      const res2 = await fetchGroqOnce({ body: makeBody(0.2, 800), timeoutMs, signal: controller.signal })
      if (res2.ok) {
