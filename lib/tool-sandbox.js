...
async function fetchGroqOnce({ body, signal }) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    // Semua key kena 429/5xx: tunggu reset (hormati Retry-After) lalu coba lagi.
    const retryAfter = Number((last || { headers: { get: () => '' } }).headers?.get?.('retry-after') || 0)
...
async function callGroqOnce({ model, systemPrompt, messages, tools, temperature, maxTokens, timeoutMs }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (isToolFail) {
        ...
        if (res.ok) {
          ...
          recordUsage(model, 'tool', d2.usage?.total_tokens)
          return {
            content: m2?.content ?? '',
    ...
    recordUsage(model, 'tool', data.usage?.total_tokens)
