...
3. hitung_math — hitung angka secara akurat lewat kode (aritmatika, desimal, akar, pangkat, jumlah huruf, faktorial fact(n), kombinasi comb(n,k), permutasi perm(n,k), bilangan Euler e). WAJIB dipakai untuk SEMUA perhitungan angka supaya jawaban 100% benar — termasuk membandingkan desimal (hitung selisihnya: hasil negatif berarti angka kedua lebih besar).
...
async function callGroqOnce({ feature = 'chat', model, systemPrompt, messages, tools, temperature, maxTokens, timeoutMs }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (isToolFail) {
        res = await fetchGroqOnce({ feature, body: makeBody(0.2, Math.max(maxTokens, 2048)), signal: controller.signal })
        if (res.ok) {
    ...
    if (truncated && maxTokens < 2048) {
      const res2 = await fetchGroqOnce({ feature, body: makeBody(0.2, 2048), signal: controller.signal })
      if (res2.ok) {
