...
async function callGroqOnce({ feature = 'chat', model, systemPrompt, messages, tools, temperature, maxTokens, timeoutMs }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (isToolFail) {
        res = await fetchGroqOnce({ feature, body: makeBody(0.2, Math.max(maxTokens, 800)), signal: controller.signal })
        if (res.ok) {
    ...
    if (truncated && maxTokens < 800) {
      const res2 = await fetchGroqOnce({ feature, body: makeBody(0.2, 800), signal: controller.signal })
      if (res2.ok) {
