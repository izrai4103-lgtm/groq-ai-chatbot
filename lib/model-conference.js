...
export async function holdConference(topic, rounds = 2) {
  ...
  const skippedModels = new Set() // Model yang di-skip karena error
  ...
  try {
    for (let round = 1; round <= rounds; round++) {
      ...
      for (const modelId of modelOrder) {
        ...
        // Skip model jika sebelumnya error terus
        if (skippedModels && skippedModels.has(modelId)) continue
        ...
        if (roundData.responses.length > 0) {
          await new Promise(r => setTimeout(r, 1500))
        }
        
        let response
        try {
          response = await callModel(config, systemPrompt, [
            { role: 'user', content: context }
          ])
        } catch (err) {
          roundData.responses.push({
            model: config.name,
            content: `[${config.name} tidak dapat merespon: ${err.message}]`,
            error: true,
          })
