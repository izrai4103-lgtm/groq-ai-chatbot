...
// ===== WEB RESEARCH (Gemini → Fallback Groq) =====
async function webResearch(topic) {
    ...
    try {
      ...
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${geminiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{ parts: [{ text: `Cari informasi terbaru tentang "${topic}" di web. Berikan fakta, data, dan sumber. Bahasa Indonesia.` }] }],
      ...
      if (res.ok) {
        ...
        if (text) return { source: '🌐 Google Gemini', content: text }
      }
    } catch (err) {
      // Fallback ke Groq
    }
  ...
  // Fallback: Groq Research model sebagai web research
  try {
    ...
    return { source: '🔍 Groq Research', content: research }
  } catch (err) {
