...
// ===== KONFIGURASI MODEL (key per fitur, lihat lib/provider-keys.js) =====
const MODELS = {
  thinking: { feature: 'thinking', label: 'Thinking Model' },
  research: { feature: 'research', label: 'Web Research' },
}
  ...
  try {
    ...
    if (!useWeb) {
      thinkResult = await runModelWithTools({
        feature: MODELS.thinking.feature,
    ...
    const researchResult = await runModelWithTools({
      feature: MODELS.research.feature,
