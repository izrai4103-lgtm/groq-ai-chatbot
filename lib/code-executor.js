...
2. Gunakan hasil web sebagai BUKTI — kutip fakta, data, dan nama sumber setelah tiap poin (contoh: (Sumber: Wikipedia)) — link lengkap ditambahkan otomatis di akhir
3. ABAIKAN hasil yang tidak relevan dengan pertanyaan (misal turnamen sepak bola 'Piala Presiden' saat user bertanya presiden negara — itu noise, jangan dibahas)
4. Kalau hasil web kurang relevan, tetap jawab berdasarkan pengetahuan umummu, lalu tandai bagian mana yang didukung hasil web
5. JANGAN pernah menjawab 'tidak ada informasi' selama kamu masih bisa menjawab — menjawab 'tidak ada informasi' padahal bisa menjawab adalah kegagalan
6. Bedakan fakta dan opini; akui keterbatasan hanya bila benar-benar tidak bisa menjawab
7. JANGAN membuat daftar link sumber sendiri di akhir jawaban — blok sumber lengkap sudah ditambahkan otomatis oleh sistem
...
export async function thinkAndResearch(question, userId = 'anonymous', useWeb = false) {
  ...
  try {
    ...
    let thinkResult = ''
    if (!useWeb) {
      thinkResult = await callModel(MODELS.thinking, THINKING_PROMPT, question)
