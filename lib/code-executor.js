...
// ===== EKSEKUSI JS PAKAI VM (AMAN) =====
function executeJavaScript(code) {
  try {
    const vm = require('vm')
    const outputs = []
    
    // Sandbox terbatas — tidak ada akses ke sistem
    const sandbox = {
...
export async function readAndExecute(code, language) {
  ...
  try {
    ...
    // Step 3: Code Executor — proses & jelaskan hasil
    if (execResult.success) {
      ...
      // Eksekusi gagal — minta Code Executor jelaskan error
      result.modelUsed += ' → Code Executor'
      try {
        const executorMsg = `Kode:\n\`\`\`\n${code}\n\`\`\`\n\nError:\n${execResult.error}\n\nJelaskan error ini dan cara memperbaikinya.`
        const executorResponse = await callModel(MODELS.executor, 
          `Jelaskan error kode ini dengan singkat dan beri saran perbaikan. Format JSON WAJIB:
{
  "error": "penjelasan error",
